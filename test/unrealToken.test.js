...
describe("UnrealToken", function(){
    ...
    it("returns correct permit hash", async function() {
        assert.equal(await unrealToken.PERMIT_TYPEHASH(),
    ...
    //     const amount = 100
    //     const nonce = await unrealToken.nonces(owner)
    ...
    //     console.log(sig);
    //     let result = await unrealToken.permit(owner, user1, amount, deadline, sig.v, sig.r, sig.s)
    ...
    //     const sig = ecsign(toBuffer(hash), toBuffer(privateKey))
    //     await assertRevert(unrealToken.permit(user1, owner, amount, deadline, sig.v, sig.r, sig.s))
    // })
