...
describe("UnrealToken Vesting", function () {
  ...
  it("should test token vesting for userX", async function () {
    ...
    time = +time;
    let result = await unrealTokenVesting.connect(accounts[0]).addVesting(accounts[1].address, time.toString(), amount);
    ...
    let balance = await unrealTokenVesting.vestingAmount(vestingId);
    assert.equal(balance.toString(), amount.toString());

    // "Tokens have not vested yet"
    await assertRevert(unrealTokenVesting.release(vestingId), "Tokens have not vested yet");
    // Time travel
    let seconds = 60 * 6000;
  ...
  it("should test addVesting data", async function () {
    ...
    time.setMinutes(time.getMinutes() + 6);
    time = +time;
    let result = await unrealTokenVesting
      .connect(accounts[0])
      .addVesting(accounts[1].address, time.toString(), vestingAmount);
    // retrieve vesting Id from events args
    const receipt = await result.wait();
    const vestingId = receipt.events[0].args.vestingId;
  ...
  it("Trying to remove a non-existent vesting entry", async function () {
    await assertRevert(unrealTokenVesting.connect(owner).removeVesting(53), "Invalid vesting id");
  });
  ...
  it("Trying to release an already released vesting entry", async function () {
    ...
    await assertRevert(unrealTokenVesting.connect(owner).release(1), "Vesting already released");
  });
  ...
  it("Trying to remove an already removed vesting entry", async function () {
    ...
    await assertRevert(unrealTokenVesting.connect(owner).removeVesting(1), "Vesting already released");
  });
  ...
  it("should test token vesting for amount greater then balance of vesting contract", async function () {
    ...
    time = +time;
    let result = await unrealTokenVesting.connect(owner).addVesting(accounts[1].address, time.toString(), amount);
    ...
    await assertRevert(unrealTokenVesting.release(vestingId), "Insufficient balance");
